...
import { dummyMenuHokiDimsum } from "@/lib/data/dummy-menu";
import { BRANDS } from "@/types/brand";
import type { MenuItem } from "@/types/menu";

export const metadata: Metadata = { title: "Hoki Dimsum" };

const brand = BRANDS.find((b) => b.slug === "hoki-dimsum")!;
const tickerItems = ["Hakau Udang", "Siomay", "Cheong Fun", "NEW MENU", "Ceker Saus Hitam", "Lumpia Shanghai", "NEW MENU", "Pao Kacang"];
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* ── 1. HERO ── */}
        <section className="relative flex min-h-screen flex-col overflow-hidden bg-[#D62828]">
          <div className="flex flex-1 flex-col justify-center px-4 pt-20 lg:px-6">
            ...
            >
              HOKI<br />
              <span className="text-[#F7B731]">DIMSUM.</span>
            </h1>
          </div>
          ...
          <div className="pointer-events-none absolute bottom-16 left-1/2 z-10 h-[55%] w-[30%] min-w-[200px] max-w-[360px] -translate-x-1/2 sm:bottom-12">
            <Image
              src="https://images.unsplash.com/photo-1563245372-f21724e3856d?w=700&q=85"
              alt="Siomay"
              fill
          ...
          {/* Badge */}
          <div className="absolute right-5 top-[40%] z-20 flex h-24 w-24 -rotate-12 select-none items-center justify-center rounded-full bg-[#F7B731] p-3 text-center text-[10px] font-black leading-tight text-[#1D1A40] sm:h-28 sm:w-28 sm:text-[11px] lg:right-10">
            Dimsum<br />Segar!
          </div>

          <div className="relative z-20 flex items-center justify-between border-t border-white/20 px-4 py-4 lg:px-6">
            <p className="text-sm text-white/60">Dimsum Premium</p>
            <div className="flex flex-wrap gap-2">
              {["Dikukus Fresh", "Isian Melimpah", "Halal"].map((t) => (
        ...
        <section className="grid grid-cols-1 md:grid-cols-2">
          <div className="relative h-64 overflow-hidden bg-[#1D1A40] md:h-96">
            ...
            <div className="absolute inset-0 flex items-end p-7">
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-white/60">Menu Andalan</p>
                <p className="mt-1 text-3xl font-black text-white" style={{ fontFamily: "var(--font-archivo)" }}>
                  Siomay<br />Ayam Udang
                </p>
              </div>
            </div>
          </div>
          <div className="flex flex-col justify-between bg-[#FFF3E8] p-7 md:p-10">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-[#70758C]">Hoki Dimsum</p>
              <h2
                className="mt-4 text-4xl font-black leading-tight text-[#1D1A40] md:text-5xl"
                style={{ fontFamily: "var(--font-archivo)" }}
              >
                Dikukus fresh,<br />
                <span className="text-[#D62828]">rezeki lancar.</span>
              </h2>
              <p className="mt-5 text-sm leading-relaxed text-[#70758C]">
                Setiap dimsum dibuat fresh dengan kulit tipis dan isian melimpah, menggunakan resep otentik khas Hong Kong.
              </p>
            </div>
            <Link href="#menu"
              className="mt-8 inline-flex w-fit rounded-full bg-[#D62828] px-6 py-3 text-sm font-bold text-white hover:opacity-80 transition-opacity"
            >
              Lihat Semua Menu ↗
            </Link>
          </div>
        ...
        {/* ── 3. TICKER ── */}
        <div className="overflow-hidden border-y-4 border-[#1D1A40] bg-[#F7B731] py-3">
        ...
        {/* ── 6. ABOUT ── */}
        <section id="tentang" className="bg-[#D62828] px-5 py-20 lg:px-8">
          ...
          <h2
            ...
            style={{ fontFamily: "var(--font-archivo)" }}
          >
            Dimsum terbaik, dibuat sepenuh hati.
          </h2>
          <p className="mt-5 max-w-md text-sm leading-relaxed text-white/70">
            Hoki Dimsum hadir untuk menghadirkan pengalaman menikmati dimsum premium — fresh harian, bahan pilihan, resep terinspirasi tradisi kuliner Tionghoa otentik.
          </p>
          <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { v: "20+", l: "Varian Dimsum" },
              { v: "4.9★", l: "Rating" },
              { v: "Fresh", l: "Dibuat Harian" },
              { v: "100%", l: "Tanpa Pengawet" },
            ].map(({ v, l }) => (
